...
import AnnouncementCard, { type AnnouncementCardData } from "../../../components/AnnouncementCard/AnnouncementCard";
 ...
 *
 * [2026-09-11] 제출 응답의 resolvedKsicCodes(diagnosis.py가 업종코드 매칭 후 확정한
 * KSIC코드)로 GET /api/matching을 한 번 더 불러서 "이런 지원사업은 어때요?" 섹션에
 * 최대 3건 보여준다 - 매칭 실패/코드 없음이면 섹션 자체를 숨긴다.
 */
function DiagnosisStep4() {
  ...
  const [matchedAnnouncements, setMatchedAnnouncements] = useState<AnnouncementCardData[]>([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const ksicCodes = data.data?.resolvedKsicCodes || [];
      if (ksicCodes.length > 0) {
        fetch(`${API_BASE_URL}/api/matching?ksic=${encodeURIComponent(ksicCodes.join(","))}&limit=3`)
          .then((r) => r.json())
          .then((body: { success: boolean; data?: AnnouncementCardData[] }) => {
            if (body.success && body.data) setMatchedAnnouncements(body.data);
          })
          .catch(() => {});
      }
    } catch {
      ...
      <div className={`pageContainer ${styles.page}`}>
        ...
        <div className={styles.scrollArea}>
          ...
          <p className={styles.noticeText}>
            답변이 저장됐어요. 상권·기술창업 분석 리포트는 완성되면 마이페이지에서
            확인하실 수 있어요.
          </p>
          {matchedAnnouncements.length > 0 && (
            <>
              <h1 className={styles.questionTitle}>이런 지원사업은 어때요?</h1>
              {matchedAnnouncements.map((item) => (
                <AnnouncementCard
                  key={item.id}
                  item={item}
                  onClick={(clicked) => navigate(`/matching/${clicked.id}`)}
                />
              ))}
            </>
          )}

...
(창업진흥원 K-Startup 수집도 2026-09-12부터 같은 이유로 로컬로 옮겨졌다.
아래 `crawl_kstartup.bat` 항목 참고.)
...
K-Startup API(`apis.data.go.kr`)는 해외 IP를 막지는 않아서 원래 GitHub Actions
(`.github/workflows/crawl-daily.yml`)에서 매일 04:07(KST)에 자동 실행됐다.

그런데 2026-09-11, 09-12 이틀 연속 실제 실행이 06:30~07:00대로 몇 시간씩 밀렸고,
그 지연된 시점에 data.go.kr 게이트웨이 응답이 없어서 크롤이 `error`로 끝났다
(`crawl_batch_logs` 테이블에서 `fetched_count=0, status='error'`로 확인됨). GitHub
Actions의 스케줄 트리거는 혼잡 시 지연이 공식적으로 보장되지 않는 best-effort라
cron 분(minute)을 정각에서 비켜도 근본적으로 해결되지 않아, kstartup도 bizinfo와
동일하게 로컬 PC 작업 스케줄러로 옮겼다.

GitHub Actions 워크플로 자체는 나중에 다시 쓸 수도 있어 삭제하지 않고 남겨뒀다
(`workflow_dispatch`로 수동 실행은 계속 가능). 다시 자동 스케줄로 되돌리려면
`.github/workflows/crawl-daily.yml`의 `schedule` 블록 주석만 풀면 된다 — 그 경우
이 로컬 작업 스케줄러 등록은 중복 실행을 피하기 위해 꺼야 한다.

### 사전 준비

- 이 저장소가 clone 되어 있고, `.venv`(파이썬 3.11 가상환경)와 `.env`가 세팅된 PC
  (= 백엔드 개발이 되는 PC면 이미 끝)
- 그 PC가 매일 04:07에 켜져 있고 로그인 상태일 것 (화면 잠금은 무방)
...
```powershell
$bat = "C:\workspaces\10_Final\mulggo-app\scripts\crawl_kstartup.bat"

$action   = New-ScheduledTaskAction -Execute $bat
$trigger  = New-ScheduledTaskTrigger -Daily -At 4:07am
$settings = New-ScheduledTaskSettingsSet -StartWhenAvailable -RunOnlyIfNetworkAvailable
Register-ScheduledTask -TaskName "mulkko-crawl-kstartup" -Action $action -Trigger $trigger `
  -Settings $settings -Description "K-Startup 원본 공고 매일 수집 (GitHub Actions 스케줄 지연 문제로 로컬 실행)"
```

- `-StartWhenAvailable`: 04:07에 PC가 꺼져 있었으면, 켜진 직후 밀린 작업을 실행한다.
- 로그인한 사용자 계정으로 실행된다 (비밀번호 저장 안 함). PC가 켜져 있고 로그인만 돼 있으면 됨.
...
```powershell
# 지금 바로 한 번 실행해보기
Start-ScheduledTask -TaskName "mulkko-crawl-kstartup"

# 마지막 실행 결과 (LastTaskResult가 0이면 성공)
Get-ScheduledTaskInfo -TaskName "mulkko-crawl-kstartup"

# 삭제
Unregister-ScheduledTask -TaskName "mulkko-crawl-kstartup" -Confirm:$false
```

...
코드: `23_match_business_code_v3.py`의 `compute_confidence()`.
...
전체 재생성이 필요할 때만:
```bash
python business_matching/21_prepare_business_code_chroma_reference.py   # 원본 CSV -> reference CSV
python business_matching/25_prepare_business_code_docs.py --example-whitelist none
        #   reference + ksic_haeseol_v1.json -> 검색문서 CSV (example/xref 포함)
python business_matching/26_build_business_code_docs_chroma.py          # 임베딩 -> 벡터 DB (OpenAI, 약 $0.05, 4~5분)
```
`25`는 `data/processed/ksic_haeseol_v1.json` 과 `ksic_clean.csv` 를 읽는다(신규 의존성).
...
1. **호출마다 재로드.** `match_business_code()`는 매번 reference CSV 로드 + Chroma 오픈 +
   OpenAI 클라이언트 생성을 한다. 트래픽 있으면 `23_match_business_code_v3.py`의
...
2. **경로.** `PROJECT_ROOT = 파일의 상위상위` 기준으로 DB·CSV·JSON을 찾는다. 패키지 폴더 구조
   ...
   경로 변경은 `23_match_business_code_v3.py` 상단 상수(`DB_DIR`, `REF_CSV`, `COLLECTION`,
...
4. **남은 오답 유형:**
   ...
   - 정답이 후보 15~31위로 깊어 재판정 창(20) 밖 → `RERANK_TOP_N` 20→28 상향 검토
     (`23_match_business_code_v3.py:57`, 2026-09-09 미검증 — OpenAI 크레딧 소진)
...
business_matching/
  23_match_business_code_v3.py   ← 파이프라인 본체 (match_business_code)
  24_eval_business_code.py       ← 성능 평가
  25_prepare_business_code_docs.py   ← 업종코드 -> 검색문서 (example/xref 포함)
  26_build_business_code_docs_chroma.py  ← 임베딩 -> 벡터 DB
  21_prepare_business_code_chroma_reference.py  ← (전체 재생성용)
  27_parse_ksic_haeseol.py       ← 해설서 PDF 파싱 -> ksic_haeseol_v1.json
  28_build_gold_from_haeseol.py  ← 해설서 근거 정답셋 생성 -> gold_set_v2.csv
  29_calibrate_confidence.py     ← 신뢰도 임계값 실측 분석 (무비용)
...
data/
  ...
  processed/example_index_whitelist.txt   빌드타임(25번)용 — 예시 선별 색인
  outputs/                          평가 결과 저장 위치
chroma_db/business_code_docs_2025/  구축된 벡터 DB (문서 14,627개, 약 225MB)
docs/

...
| 파일 | 역할 | 비용 |
|---|---|---|
| `business_matching/23_match_business_code_v3_baseline_20260909.py` | v3 동결본. **수정 금지.** 롤백·대조용. | — |
| `business_matching/23_match_business_code_v3.py` | 상수 블록 + `hierarchy_fit()` + `build_candidate_pool()` 5줄. 플래그 기본 off. | — |
...
```
# 코드 전체를 v3 원본으로:
cp business_matching/23_match_business_code_v3_baseline_20260909.py \
   business_matching/23_match_business_code_v3.py
```

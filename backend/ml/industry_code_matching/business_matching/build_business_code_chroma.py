...
# =============================================================================
# build_business_code_chroma.py
#
# 목적
# - `prepare_business_code_chroma_reference.py`에서 만든 업종코드 reference CSV를 읽는다.
# - embedding_text를 OpenAI text-embedding-3-small로 임베딩한다.
# - 6자리 업종코드 1개 = Chroma 문서 1개 구조로 저장한다.
# - 저장 위치: chroma_db/business_codes_2025/
#
# 중요
# - `match_business_code.py` 검색에서도 반드시 같은 Embedding 모델을 사용해야 한다.
...
def load_reference(path: Path) -> pd.DataFrame:
    """`prepare_business_code_chroma_reference.py` 출력 파일을 읽고 구조를 검증한다."""
    if not path.exists():
        raise FileNotFoundError(
            "`prepare_business_code_chroma_reference.py` 출력 파일을 찾을 수 없습니다.\n"
            f"확인 경로: {path}\n\n"
            "먼저 prepare_business_code_chroma_reference.py를 실행하세요."
        )
    ...
    if df["business_code"].duplicated().any():
        ...
        raise ValueError(
            "업종코드 중복이 발견되었습니다.\n"
            "`prepare_business_code_chroma_reference.py` 결과는 '업종코드 1개 = 1행'이어야 합니다.\n"
            f"예시: {duplicated}"
        )
...
def reset_collection(
    ...
    `build_business_code_chroma.py`를 다시 실행했을 때 과거 DB와 섞이는 것을 방지한다.
    """
    try:
...
def build_chroma(
    ...
    total = len(df)
    print("\n" + "=" * 72)
    print("build_business_code_chroma.py: ChromaDB 구축 시작")
    print("=" * 72)
    ...
    print("\n" + "=" * 72)
    print("build_business_code_chroma.py: ChromaDB 구축 완료")
    print("=" * 72)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"`prepare_business_code_chroma_reference.py` reference CSV (기본값: {DEFAULT_INPUT})",
    )
    ...
    if args.model != DEFAULT_EMBEDDING_MODEL:
        ...
        print(
            "`build_business_code_chroma.py`와 `match_business_code.py`의 Embedding 모델은 반드시 같아야 합니다.\n"
        )

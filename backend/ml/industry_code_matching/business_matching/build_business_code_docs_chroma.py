...
# =============================================================================
# build_business_code_docs_chroma.py
#
# 목적
# - `prepare_business_code_docs.py`가 만든 business_code_docs_v1.csv (업종코드 1개 = 문서 여러 개)를
...
# - `match_business_code.py`는 이 새 collection을 바라보도록 이미 수정되어 있다.
# =============================================================================
...
def load_docs(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(
            f"prepare_business_code_docs.py 출력 파일이 없습니다: {path}\n"
            "먼저 prepare_business_code_docs.py를 실행하세요."
        )
...
def build(df: pd.DataFrame, db_dir: Path, collection_name: str, model: str, batch_size: int) -> None:
    ...
    total = len(df)
    print("\n" + "=" * 72)
    print("build_business_code_docs_chroma.py: ChromaDB(다중문서) 구축 시작")
    print("=" * 72)
    print(f"문서 수        : {total:,}")
    ...
    print("\n" + "=" * 72)
    print("build_business_code_docs_chroma.py: 구축 완료")
    print("=" * 72)

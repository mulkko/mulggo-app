...
# =============================================================================
# match_business_code.py  (단순화 유지 + 검색 품질 개선판)
#
# 개선 포인트 (2026-09)
...
def validate_inputs(seed: str, problem: str, solution: str) -> None:
    for name, value in [("seed", seed), ("problem", problem), ("solution", solution)]:
...
위 정보만 사용해서 실제 수익활동을 1~3개로 구조화하라.
지역 정보는 사용하지 않는다.
""".strip()
    ...
    resp = client.chat.completions.create(
        ...
        response_format={"type": "json_schema", "json_schema": ACTIVITY_SCHEMA},
    )

    activities = json.loads(resp.choices[0].message.content)["activities"]
...
def vector_search_multi(
    ...
    """
    embs = client.embeddings.create(model=embedding_model, input=queries).data
    embs = [e.embedding for e in sorted(embs, key=lambda x: x.index)]
...
def build_candidate_pool(
    ...
    for x in pool:
        ...
        x["role_adjusted_rank"] = base_rank + _ROLE_RANK_DELTA.get(x.get("role_fit", 1), 0)
...
def compute_confidence(
    ...
    calibration (`calibrate_confidence.py`, 2026-09-09):
...
def match_business_code(
    ...
    seed = clean(seed)
    problem = clean(problem_to_solve)
    solution = clean(solution_approach)
    region = clean(region)

    validate_inputs(seed, problem, solution)

    llm_model, embedding_model = load_env()
    client = OpenAI(api_key=os.environ["OPENAI_API_KEY"])

    ref_df = load_reference()
    lookup = reference_lookup(ref_df)
    collection = open_chroma(embedding_model)

    activities = extract_activities(client, llm_model, seed, problem, solution)
    ...
    for activity in activities:
        ...
        pool = build_candidate_pool(v, k, lookup, activity)

        if use_llm_rerank:
            rr = rerank(client, llm_model, activity, pool)
        else:
            top1 = pool[0]
            rr = {
...
def print_result(result: dict[str, Any]) -> None:
    print("\n" + "=" * 88)
    print("match_business_code_baseline_20260909.py: 업종코드 매칭 - v3 (검색 품질 개선, 동결본)")
    print("=" * 88)
...
def interactive_inputs() -> tuple[str, str, str, str]:
    seed = input("\n슬롯 0 사업아이디어:\n> ").strip()
    problem = input("\n슬롯 2 문제/기회:\n> ").strip()
    solution = input("\n슬롯 5 사업화 방식:\n> ").strip()
    region = input("\n슬롯 14 지역(선택, 업종 검색 미사용):\n> ").strip()
    return seed, problem, solution, region
...
def main() -> None:
    ...
    if args.seed and args.problem and args.solution:
        values = (args.seed, args.problem, args.solution, args.region)
    else:
        values = interactive_inputs()

    result = match_business_code(
        seed=values[0],
        problem_to_solve=values[1],
        solution_approach=values[2],
        region=values[3],

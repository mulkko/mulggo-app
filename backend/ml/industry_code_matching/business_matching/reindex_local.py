"""
reindex_local.py — business_code_docs_v1.csv 를 로컬 임베딩 모델로 재색인.

`build_business_code_docs_chroma.py`(OpenAI)과 동일 구조, OpenAI 호출 없음. GPU 사용.
기존 OpenAI 컬렉션(business_code_docs_2025)은 건드리지 않는다 — A/B 비교용으로 둘 다 유지.

사용:
    python business_matching/reindex_local.py --model bge-m3
    python business_matching/reindex_local.py --model bge-m3 --limit 200   # 스모크
...
def main() -> None:
    ...
    if embedders.is_openai(args.model):
        raise SystemExit(f"'{args.model}' 은 OpenAI 모델입니다. build_business_code_docs_chroma.py를 쓰세요.")
    ...
    print("=" * 72)
    print(f"reindex_local.py: 로컬 재색인  |  모델={args.model}  차원={embedders.embedding_dim(args.model)}")

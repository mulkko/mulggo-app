...
#     · def     : KSIC 11차 해설서 '정의문'            ← 공식 설명 (`parse_ksic_haeseol.py`)
#     · example : KSIC 11차 해설서 <예시> 활동 문구      ← 구체 활동, 반직관 케이스 포함 (`parse_ksic_haeseol.py`)
#
...
#   안 담겨 있었다 (`parse_ksic_haeseol.py`는 정답셋 생성용으로만 썼음).
...
# - 원본/`prepare_business_code_chroma_reference.py` reference CSV는 건드리지 않는다. 여기서는 그걸 읽어 docs CSV만 새로 만든다.
#
# 입력 : data/processed/business_code_chroma_reference_v1.csv   (`prepare_business_code_chroma_reference.py` 출력)
#        data/processed/ksic_haeseol_examples_v1.csv            (`parse_ksic_haeseol.py` 출력)
#        data/processed/ksic_haeseol_v1.json                    (`parse_ksic_haeseol.py` 출력, 정의문)
...
SEP = " | "  # `prepare_business_code_chroma_reference.py`가 다중 값을 이 구분자로 합쳐 놓았다
...
def load_haeseol_examples(path: Path, whitelist: set[str] | None) -> dict[str, list[str]]:
    """
    `parse_ksic_haeseol.py`의 examples CSV -> {업종코드: [예시활동 문구, ...]}.
...
def load_haeseol_excludes(path: Path, k2b: dict[str, set[str]]) -> dict[str, list[str]]:
    """
    `parse_ksic_haeseol.py` JSON의 <제외> 규칙 -> {업종코드: [그 코드로 '가야 하는' 활동 문구, ...]}.
...
def load_haeseol_definitions(path: Path) -> dict[str, str]:
    """
    `parse_ksic_haeseol.py`의 haeseol JSON -> {업종코드: 정의문}.
...
def main() -> None:
    ...
    if not args.input.exists():
        raise FileNotFoundError(f"prepare_business_code_chroma_reference.py의 reference CSV가 없습니다: {args.input}")
    ...
    print("\n" + "=" * 70)
    print("prepare_business_code_docs.py: 문서 확장 완료")
    print("=" * 70)

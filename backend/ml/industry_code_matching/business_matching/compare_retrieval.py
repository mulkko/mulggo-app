"""
compare_retrieval.py — 개정 계획서 §6.4 / §10.5(A·B).

"사업설명 원문 -> 임베딩 -> 검색"만으로 정답 업종코드가 상위 K에 들어오는지 측정.
LLM 안 씀. 로컬 임베딩(bge-m3)이면 크레딧 0.

임베딩 모델별로 따로 돌려서 비교:
    python business_matching/compare_retrieval.py --model bge-m3
    python business_matching/compare_retrieval.py --model text-embedding-3-small
...
BASE_DOC_TYPES = ["name", "hier", "ksic", "desc", "def"]   # `match_business_code.py`와 동일 (공식 정보 채널)

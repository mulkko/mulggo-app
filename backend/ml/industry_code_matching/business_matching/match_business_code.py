...
# 게이트 = 앙상블 (ensemble_gate.py, 사람검수 골든셋 gold_set_v3 90건):
...
def load_env() -> tuple[str, str]:
    # 이 패키지 전용 .env는 없음 — 프로젝트 루트 .env(OPENAI_API_KEY 포함)를 그대로 씀
    # (python-dotenv가 현재 위치에서 상위로 올라가며 자동으로 찾음).
    # LLM_MODEL/EMBEDDING_MODEL은 backend/ml/classifier/llm_match.py가 이미 같은 이름을
    # 다른 기본값(gpt-4o-mini)으로 쓰고 있어서, 루트 .env에 그냥 합치면 그쪽 동작까지
    # 조용히 바뀐다 — 그래서 이 패키지만 INDUSTRY_MATCH_ 접두사로 구분해서 읽는다.
    ...
    return (
        os.getenv("INDUSTRY_MATCH_LLM_MODEL", DEFAULT_LLM_MODEL).strip(),
        os.getenv("INDUSTRY_MATCH_EMBEDDING_MODEL", DEFAULT_EMBEDDING_MODEL).strip(),
    )
...
def decide_result_state(
    ...
    추천_가능      : 신뢰도 high AND 원문검색 1위 == 재판정 선택 AND 역할 경계 아님
                     (앙상블. 두 독립 신호 일치 = 88% 정확 / 34% 커버. ensemble_gate.py)
...
def print_result(result: dict[str, Any]) -> None:
    print("\n" + "=" * 88)
    print("match_business_code.py: 업종코드 매칭 (검색 품질 개선)")
    print("=" * 88)

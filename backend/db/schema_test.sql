...
-- best_practices_summary는 별도 진행 중인 업종코드 매칭·분석 기능이 채우는 자리 -
-- diagnosis.py는 NULL로 둠.
--
-- [2026-09-11] resolved_ksic_codes 컬럼 신규 - 아직 실 DB(Supabase)엔 없음, 만들기만
-- 해두고 적용은 나중에. 국세청업종코드(resolved_nts_codes)와 KSIC코드 체계가 서로
-- 달라서(backend/ml/industry_code_matching 참고 - 매칭기는 국세청코드를 뱉고, 참조
-- 테이블의 linked_ksic_codes로 KSIC코드를 함께 얻음) 같은 JSONB 안에 섞지 않고 컬럼을
-- 분리했다 - analysis.py(/analysis/market, /analysis/tech-startup)와 matching.py
-- (/api/matching?ksic=)가 전부 KSIC코드를 받기 때문에 이 컬럼 값을 바로 넘기면 된다.
-- 적용 시 실행할 SQL: ALTER TABLE idea_refinement_sessions ADD COLUMN IF NOT EXISTS
-- resolved_ksic_codes JSONB;
CREATE TABLE IF NOT EXISTS idea_refinement_sessions (
    ...
    resolved_ksic_codes      JSONB,                         -- [신규, 미적용] 국세청코드에 연계된 KSIC코드 배열
    region                   TEXT,
